...
export class EditUserActionPage {
  ...
  modifyUser() {
    ...
    loader.present().then(() => {
      this.http.post('http://172.10.20.169/IonicApp/edit_user.php', data, options)
        .map(res => res.json())
        .subscribe(res => {
  ...
  getUsers() {
    this.httpC.get("http://172.10.20.169/IonicApp/json_fetch_charge.php").subscribe(userData => {
