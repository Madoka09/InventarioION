...
export class ProductEntryPage {
  ...
  getProviders(){
    this.httpC.get("http://192.168.137.1/IonicApp/json_fetch_providers.php").subscribe(data =>{
  ...
  generateEntry() {
    ...
    if (this.nuevaCantidad.value == "" || this.nuevaCantidad.value == String) {
      ...
      loader.present().then(() => {
        this.http.post('http://192.168.137.1/IonicApp/json_entry_stock.php', data, options)
          .map(res => res.json())
          .subscribe(res => {
